...
function AuthProvider({ children }) {
  ...
  // Load doctors for patient to choose who receives their report
  useEffect(() => {
    (async () => {
      try {
        ...
        try {
          ...
          if (saved) setSelectedDoctorId_Q(saved || null);
        } catch (_) {}
      } catch (e) {
...
function QuestionnairePage() {
  ...
  const [includeVitals_Q, setIncludeVitals_Q] = useState(true);
  const [includeUploads_Q, setIncludeUploads_Q] = useState(true);
  const [previewData_Q, setPreviewData_Q] = useState(null);
  const [previewLoading_Q, setPreviewLoading_Q] = useState(false);
  const [doctors_Q, setDoctors_Q] = useState([]);
  const [selectedDoctorId_Q, setSelectedDoctorId_Q] = useState(null);
    ...
    try {
      ...
      try {
        ...
        if (uid) {
          await supabase.from(TBL_REPORT).insert([{
            patient_id: uid,
            doctor_id: selectedDoctorId_Q || null,
            content: j.report || '',
            ai_generated: true,
  ...
  async function previewContext_Q() {
    setPreviewLoading_Q(true);
    setPreviewData_Q(null);
    try {
      const ctx = await buildInterviewContext();
      const filtered = {
        vitals: includeVitals_Q ? ctx.vitals : [],
        uploads: includeUploads_Q ? ctx.uploads : []
      };
      setPreviewData_Q(filtered);
    } catch (e) {
      setPreviewData_Q({ error: String(e?.message || e) });
    } finally {
      setPreviewLoading_Q(false);
    }
  }
  ...
  <div className="card questionnaire-container route-screen">
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ minWidth: 220 }}>
                  <label style={{ display: 'block', fontSize: 12, marginBottom: 6 }}>Doctor to receive report</label>
                  <select value={selectedDoctorId_Q || ''} onChange={e => {
                    const v = e.target.value || null;
                    setSelectedDoctorId_Q(v);
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <input type="checkbox" checked={includeVitals_Q} onChange={e => setIncludeVitals_Q(e.target.checked)} /> Vitals
              </label>
              <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <input type="checkbox" checked={includeUploads_Q} onChange={e => setIncludeUploads_Q(e.target.checked)} /> Uploads
              </label>
              <button className="btn btn-light" onClick={previewContext_Q} disabled={previewLoading_Q}>{previewLoading_Q ? 'Previewing…' : 'Preview Context'}</button>
              <button
        ...
        {error && <div className="alert alert-danger">{error}</div>}
        {previewData_Q && (
          <div style={{ marginBottom: 12, padding: 8, background: '#fff8e6', border: '1px solid #f2d6a0', borderRadius: 6 }}>
            <strong>Preview (vitals & uploads):</strong>
            {previewData_Q.error ? <div style={{ color: '#a00' }}>{previewData_Q.error}</div> : (
              <pre style={{ whiteSpace: 'pre-wrap', marginTop: 8 }}>{JSON.stringify(previewData_Q, null, 2)}</pre>
            )}
          </div>
        )}
