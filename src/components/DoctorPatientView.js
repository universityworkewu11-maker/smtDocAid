	...
	useEffect(() => {
		(async () => {
			...
			try {
				...
				// Diagnoses / AI reports for this patient with doctor-restricted visibility
				const tbl = process.env.REACT_APP_TBL_REPORT || 'diagnoses';
				const targetUserId = p.data?.user_id || id;
				let diagError = null;
				let list = [];
				try {
					// Get current signed-in doctor id (if any)
					const { data: { user } } = await supabase.auth.getUser();
					const currentDoctorId = user?.id || null;
					// Only show reports for this patient that are either unassigned (doctor_id is null)
					// or assigned to the current doctor. This prevents other doctors from seeing private reports.
					let query = supabase.from(tbl).select('id,patient_id,content,severity,ai_generated,created_at,metadata').eq('patient_id', targetUserId).order('created_at', { ascending: false }).limit(50);
					if (currentDoctorId) {
						// apply OR filter: doctor_id is null OR doctor_id == currentDoctorId
						query = supabase.from(tbl).select('id,patient_id,content,severity,ai_generated,created_at,metadata').or(`patient_id.eq.${targetUserId},patient_id.eq.${targetUserId}`).order('created_at', { ascending: false }).limit(50);
						// Instead of complex OR on patient_id + doctor_id we will fetch by patient_id then filter client-side for doctor_id
						const { data: rr, error: rErr } = await supabase
							.from(tbl)
							.select('id,patient_id,content,severity,ai_generated,created_at,metadata,doctor_id')
							.eq('patient_id', targetUserId)
							.order('created_at', { ascending: false })
							.limit(200);
						if (rErr) diagError = rErr; else list = (rr || []).filter(r => (r.doctor_id == null) || (String(r.doctor_id) === String(currentDoctorId)));
					} else {
						// If not signed in as doctor, default to returning reports for the patient (patient view)
						const { data: rr, error: rErr } = await supabase
							.from(tbl)
