            ...
            function AIQuestionnairesPage() {
                ...
                async (path, body = {}) => {
                  ...
                  try {
                    ...
                    if (!serverBase || serverBase !== 'http://localhost:5001') {
                      ...
                      try { window.localStorage.setItem(LS_KEYS.base, fallbackBase); } catch (_) {}
                      return json;
                    }
                    throw primaryError;
                  }
              ...
              const startInterview = async () => {
                ...
                try {
                  ...
                  setContextData(ctx);
                  persistContext(ctx);
                  const response = await apiPostJSON('/api/v1/ai/interview/start', { context: ctx, language: interviewLanguage });
                  setInterview({ sessionId: response.sessionId, question: response.question || '', turns: [], done: Boolean(response.done), report: '' });
                  persistInterview({ sessionId: response.sessionId, question: response.question || '', turns: [], done: Boolean(response.done), report: '' });
                  setIAnswer('');
                } catch (startError) {
