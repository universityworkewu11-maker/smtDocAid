...
const VitalsPage = () => {
  ...
  // Cache resolved base so we don't probe repeatedly
  let rpiBaseCache = null;
  ...
  const findRpiBase = async () => {
    if (rpiBaseCache) return rpiBaseCache;
    const env = (process.env.REACT_APP_RPI_API_BASE || '').replace(/\/$/, '');
    const candidates = [];
    if (env) candidates.push(env);
    // prefer localhost first, then raspberrypi.local, then local Pi IP
    candidates.push('http://127.0.0.1:7000');
    candidates.push('http://raspberrypi.local:7000');
    candidates.push('http://10.70.216.96:7000');

    for (const base of candidates) {
      try {
        const res = await probeUrl(`${base}/api/health`, 1200);
        if (res && res.ok) {
          rpiBaseCache = base;
          return base;
        }
    ...
    rpiBaseCache = candidates[candidates.length - 1];
    return rpiBaseCache;
  };
  ...
  const takeVitalReading = async (vitalType, pinNumber) => {
    ...
    try {
      const RPI_BASE = await findRpiBase();

      // 1) hit /api/health to ensure sampling starts
      try {
        await fetch(`${RPI_BASE}/api/health`);
      } catch (_) {}

      // 2) call read endpoint with wait parameter for sensors that may need time
      const wait = vitalType === 'temperature' ? 0 : 3; // heartRate/spo2 can take longer
      const sensorPath = vitalType === 'heartRate' ? 'heartRate' : (vitalType === 'spo2' ? 'spo2' : 'temperature');
      const url = `${RPI_BASE}/api/read/${sensorPath}?pin=${pinNumber}&wait=${wait}`;
      ...
      if (vitalType === 'heartRate') {
        value = j.heart_rate_bpm;
        valid = j.valid;
      } else if (vitalType === 'spo2') {
        value = j.spo2_percent;
        valid = j.valid;
      } else if (vitalType === 'temperature') {
        value = j.object_temp_C; // or ambient_temp_C
        valid = j.connected;
      }
